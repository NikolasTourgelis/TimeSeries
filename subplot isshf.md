...
# Extract the 'ishf' variable (Instantaneous Surface Sensible Heat Flux)
...
months_athens, ishf_monthly_athens = compute_monthly_average(dates, ishf_athens)
months_thessaloniki, ishf_monthly_thessaloniki = compute_monthly_average(dates, ishf_thessaloniki)
...
axs[0].plot(months_athens, ishf_monthly_athens, label='Athens', color='blue')
axs[0].plot(months_thessaloniki, ishf_monthly_thessaloniki, label='Thessaloniki', color='red')
axs[0].xaxis.set_major_locator(mdates.MonthLocator())
axs[0].xaxis.set_major_formatter(mdates.DateFormatter('%b %Y'))
axs[0].set_title('Monthly Avg Instantaneous Surface Sensible Heat Flux')
axs[0].set_xlabel('Month')
axs[0].set_ylabel('Monthly Avg ISHF (W/m²)')
axs[0].legend()
...
axs[1].bar(positions_athens, ishf_seasonal_athens, width=bar_width, color='blue', alpha=0.7, label='Athens')
axs[1].bar(positions_thessaloniki, ishf_seasonal_thessaloniki, width=bar_width, color='red', alpha=0.7, label='Thessaloniki')
axs[1].set_xticks(np.arange(len(seasons_athens)))
axs[1].set_xticklabels(seasons_athens)
axs[1].set_title('Seasonal Avg Instantaneous Surface Sensible Heat Flux')
axs[1].set_xlabel('Season')
axs[1].set_ylabel('Average ISHF (W/m²)')
axs[1].legend()
